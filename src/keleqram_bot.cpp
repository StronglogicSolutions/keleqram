...
namespace keleqram
{
  ...
  { if (config.ParseError() < 0) throw std::invalid_argument{"Config parse failed"}; return true; }();
  ...
  static std::string Greeting(MessagePtr &message)
  {
    ...
    const auto &name = (message->newChatMember) ? (message->newChatMember->firstName.empty()) ? message->from->firstName : message->newChatMember->firstName : (message->from) ? message->from->firstName
                                                                                                                                                                               : "the room";
  ...
  static std::string HandleRequest(std::string message)
  {
    ...
    else if (ToLower(message).find("/kanye") != std::string::npos)
  ...
  */
  ...
  {
    if (m_rooms.empty())
      throw std::invalid_argument{"Please add rooms to config file"};
  ...
  void KeleqramBot::SendMedia(const std::string &url, const T &id)
  {
    ...
    auto is_local = [&url]
    { return url.find("file://") != std::string::npos; };
    auto strpprot = [&url]
    ...
    tx_msgs[dest].emplace_back((mime.IsPhoto()) ? m_api.sendPhoto(dest, TgBot::InputFile::fromFile(path, mime.name))->messageId : m_api.sendVideo(dest, TgBot::InputFile::fromFile(path, mime.name))->messageId);
  }
  ...
  void KeleqramBot::HandleMessage(MessagePtr message)
  {
    ...
    auto IsEvent = [](const MessagePtr &message) -> bool
    { return message->text.empty(); };
    auto Broadcast = [this](const auto v) -> void
    { for (const auto& fn : m_observers) fn(v); };
    auto IsPollResult = []() -> bool
    { return false; };
    auto GetPollData = []() -> Payload
    { return Payload{}; };
  ...
  {
    if (!GroupHandlesEvents(message->chat->id))
      return;
